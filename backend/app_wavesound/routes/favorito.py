...
def agregar_favorito(
    ...
    return favoritos_services.agregar_favorito(
        db,
...
def eliminar_favorito(
    ...
    return favoritos_services.eliminar_favorito(
        db,
        usuario_actual.id_usuario,
        id_cancion
    )
...
# ✅ LISTAR FAVORITOS (SIN response_model)
@router.get("/")
def listar_favoritos(
    db: Session = Depends(get_db),
    usuario_actual = Depends(get_current_user)
):
    return favoritos_services.listar_favoritos_usuario(
        db,
        usuario_actual.id_usuario
    )
...
def get_likes_cancion(
    id_cancion: int,
    db: Session = Depends(get_db),
    usuario_actual = Depends(get_current_user)
):
    return favoritos_services.obtener_likes_cancion(
        db,
        id_cancion,
        usuario_actual.id_usuario
    )
